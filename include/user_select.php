...
<!-- -->
    ...
    if (isset($single_user_select_field_id))
            ...
            <td colspan="2" align="left">
                ...
                <?php
                    if ($found_single_user_select_field_value!='')
                        {
                        ?><script>jQuery("#<?php echo escape($autocomplete_user_scope); ?>adduserbutton").attr('value', '<?php echo escape($lang["clearbutton"]); ?>');</script><?php
                        }
                ?>
                <input type="hidden" id="<?php echo escape($single_user_select_field_id); ?>" name="<?php echo escape($single_user_select_field_id); ?>" value="<?php
                    if(isset($single_user_select_field_value)) { echo escape($single_user_select_field_value); } ?>" />
            </td>
        </tr>
        <?php
        }
        ...
        <!-- user string -->
        <tr>
            <td colspan="2" align="left"><textarea rows=6 class="stdwidth" name="<?php echo escape($autocomplete_user_scope); ?>users" id="<?php echo escape($autocomplete_user_scope); ?>users"
                                                   <?php if (!$sharing_userlists){ ?>onChange="this.value=this.value.replace(/[^,] /g,function replacespaces(str) {return str.substring(0,1) + ', ';});"
                                                   <?php } else { ?>onChange="<?php echo escape($autocomplete_user_scope); ?>addUser();<?php echo escape($autocomplete_user_scope); ?>checkUserlist();<?php echo escape($autocomplete_user_scope); ?><?php echo escape($autocomplete_user_scope); ?>updateUserSelect();"<?php } ?>><?php echo escape($userstring); ?></textarea>
            </td>
        </tr>
        <!-- -->
    <?php
    ...
    <div id="<?php echo escape($autocomplete_user_scope); ?>userlist_name_div" style="display:none;">
        <input type="text" class="medwidth" value="<?php echo escape($lang['typeauserlistname']); ?>"  id="<?php echo escape($autocomplete_user_scope); ?>userlist_name_value" name="userlist_parameter" onClick="this.value='';" /></div>
    </td>

    <td>
    <div id="<?php echo escape($autocomplete_user_scope); ?>userlist_+" style="display:none;"><input type=button value="<?php echo escape($lang['saveuserlist']); ?>" class="medcomplementwidth" onClick="<?php echo escape($autocomplete_user_scope); ?>saveUserList();" />
    </td></tr>

    <tr><td>
        <select id="<?php echo escape($autocomplete_user_scope); ?>userlist_select" class="medwidth" onchange="document.getElementById('<?php echo escape($autocomplete_user_scope); ?>users').value=document.getElementById('<?php echo escape($autocomplete_user_scope); ?>userlist_select').value;document.getElementById('<?php echo escape($autocomplete_user_scope); ?>userlist_name_div').style.display='none';document.getElementById('<?php echo escape($autocomplete_user_scope); ?>userlist_+').style.display='none';if (document.getElementById('<?php echo escape($autocomplete_user_scope); ?>userlist_select').value==''){document.getElementById('<?php echo escape($autocomplete_user_scope); ?>userlist_delete').style.display='none';}else{document.getElementById('<?php echo escape($autocomplete_user_scope); ?>userlist_delete').style.display='inline';}"></select>
    </td>
...
function <?php echo escape($autocomplete_user_scope); ?>addUser(event,ui)
    ...
    if (username.indexOf("<?php echo escape($lang["group"]); ?>") != -1)
        {
        if ((confirm("<?php echo escape($lang["confirmaddgroup"]); ?>"))==false) {return false;}
        }
    if (username.indexOf("<?php echo escape($lang["groupsmart"]); ?>")!=-1)
        {
        if ((confirm("<?php echo escape($lang["confirmaddgroupsmart"]); ?>"))==false) {return false;}
        }
