...
if ($plugin) {
    ...
    $breadcrumbs = [
        ...
        ['title' => $lang['whisper_configuration'], 'href' => "{$baseurl_short}plugins/whisper/pages/setup.php", 'menu' => false],
        ['title' => $lang["job_configure"] . ": " . $lang["whisper_process_existing"] . " (" .$lang["whisper"] . ")"]
    ];
} else {
    if ($job_user == $userref) {
...
if ($save != '' && enforcePostRequest(false)) {
    ...
    if ($valid_data) {
        ...
        // Create the job
        $job_added = job_queue_add("process_whisper", $job_data);
        
        if (!is_int_loose($job_added)) {
            $job_add_error = true;
        } else {
            // Log that job has been added to user activity log, and then redirect
            log_activity("Added process_whisper job $job_added", 
                            LOG_CODE_JOB_ADDED,
...
<div class='BasicsBox'>
    <h1>
        <?php
        echo escape($lang["job_configure"] . ": " . $lang["whisper_process_existing"] . " (" .$lang["whisper"] . ")");
    ...
    <p><?php echo escape( $lang["whisper_process_existing_intro"]); ?></p>

    <form method="post" id="process_whisper_form" action="<?php echo generateURL("{$baseurl_short}plugins/whisper/pages/offline_jobs/process_whisper.php", ["job_user" => $job_user, "plugin" => $plugin]); ?>">
        <?php generateFormToken("process_whisper_form"); ?>
