...
class MyService(win32serviceutil.ServiceFramework):
    _svc_name_ = "RS CLIP Service"
    _svc_display_name_ = "RS CLIP Service"
    _svc_description_ = "CLIP search service for ResourceSpace"
    ...
    def main(self):
        ...
        config_path = os.path.join(script_dir, "clip_config.ini")

        config = configparser.ConfigParser()
        config.read(config_path)

        svc_section = "CLIP Service"
        db_section = "Database"
        ...
        dbpass = config.get(db_section, "dbpass")
        args = ["--dbuser", dbuser, "--dbpass", dbpass]

        creationflags = subprocess.CREATE_NEW_PROCESS_GROUP
        log_file = os.path.join(script_dir, "clip_service.log") if enable_logging else os.devnull
