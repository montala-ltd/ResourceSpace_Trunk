...
if ($plugin) {
    ...
    $breadcrumbs = [
        ...
        ['title' => $lang["job_configure"] . ": " . $lang["faces_tag_faces"]]
    ];
} else {
    if ($job_user == $userref) {
...
<div class='BasicsBox'>
    <h1>
        <?php
        echo escape($lang["job_configure"] . ": " . $lang["faces_tag_faces"]);
    ...
    <form method="post" id="faces_tag_form" action="<?php echo generateURL("{$baseurl_short}plugins/faces/pages/offline_jobs/faces_tag.php", ["job_user" => $job_user, "plugin" => $plugin]); ?>">
        <?php generateFormToken("faces_tag_form"); ?>
        <div class="Question<?php
            ...
            <div class="FormHelp JobFormHelp">
                <div class="FormHelpInner"><?php echo escape($lang["faces_tag_faces_collection_refs_help"]); ?></div>
            </div>
