...
<div class="BasicsBox">
    ...
    <form id='myform' action="<?php echo $baseurl_short?>pages/collection_download.php?id=<?php echo urlencode($uniqid) ?>&submitted=true" method=post>
        ...
        <?php if ($count_data_only_types !== count($result)) { ?>
            <div class="Question">
        ...
        if (
            ...
            <div class="Question">
                ...
                <input type=checkbox id="use_original" name="use_original" value="yes">
                <div class="clearerleft"></div>
            </div>
            <?php
        }
        ...
        if ($zipped_collection_textfile == "true") {
            ...
            <div class="Question">
                ...
                <select name="text" class="shrtwidth" id="text"<?php echo (!empty($submitted)) ? ' disabled="disabled"' : ''; ?>>
        ...
        if ($archiver && count($collection_download_settings) > 1) { ?>
            <div class="Question" id="archivesettings_question" <?php echo $collection_download_tar ? "style=\"display:none\"" : ''; ?>>
                <label for="archivesettings"><?php echo escape($lang["archivesettings"]); ?></label>
                <div class="tickset">
                    <select name="settings" class="stdwidth" id="archivesettings"<?php echo (!empty($submitted)) ? ' disabled="disabled"' : ''; ?>>
                        <?php foreach ($collection_download_settings as $key => $value) { ?>
                            <option value="<?php echo escape($key) ?>"><?php echo lang_or_i18n_get_translated($value["name"], "archive-") ?></option>
                        <?php } ?>
                    </select>
                    <div class="clearerleft"></div>
                </div>
