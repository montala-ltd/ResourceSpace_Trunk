                    ...
                    <div class="tickset">
                        <div class="Inline">
                            <select name="group" id="group" onChange="this.form.submit();">
                                <option value="0"<?php if ($group == 0) { echo " selected"; } ?>><?php echo escape($lang["all"]); ?></option>
                                <?php
                                for($n=0;$n<count($groups);$n++){
                                    ?>
                                    <option value="<?php echo (int) $groups[$n]["ref"]; ?>"<?php if ($group == $groups[$n]["ref"]) { echo " selected"; } ?>><?php echo escape($groups[$n]["name"]); ?></option>
                                    <?php
                                }
                                ?>
                            </select>
...
<div class="BasicsBox"> 
    ...
    <div class="TopInpageNav"><div class="TopInpageNavLeft"><?php echo $atoz?>  <div class="InpageNavLeftBlock"><?php echo escape($lang["resultsdisplay"])?>:
        <?php 
        ...
        if ($per_page==99999)
            {
            ?><span class="Selected"><?php echo escape($lang["all"])?></span><?php
            }
        else
            {
            $url=generateURL(
    ...
    for($n = 0; ($n < count($users) && $n < ($offset + $per_page)); $n++)
        ...
        <tr>
            <td>
                <div class="ListTitle">
                    <a href="<?php echo $team_user_edit_url; ?>" onClick="return CentralSpaceLoad(this, true);"><?php echo escape($users[$n]["username"]); ?></a>
                </div>
            </td>
    ...
    <strong><?php echo escape($lang["total"] . ": " . $results); ?> </strong><?php echo escape($lang["users"]); ?>
    </div>
...
if ($user_purge)
    ...
    ?>
    <div class="BasicsBox">
    <div class="Question"><label><?php echo escape($lang["purgeusers"])?></label>
    <div class="Fixed"><a onClick="return CentralSpaceLoad(this,true);" href="<?php echo $baseurl ?>/pages/team/team_user_purge.php"><?php echo LINK_CARET ?><?php echo escape($lang["purgeusers"])?></a></div>
...
<div class="BasicsBox">
<div class="Question"><label><?php echo escape($lang["usersonline"])?></label>
<div class="Fixed">
<?php
$active=get_active_users();
for ($n=0;$n<count($active);$n++) {if($n>0) {echo", ";}echo "<b><a href='" . $baseurl . "/pages/team/team_user_edit.php?ref=" . $active[$n]["ref"] . "&backurl=" . urlencode($url . "&offset=" . $offset) . "' onClick='return CentralSpaceLoad(this,true);'>" . escape($active[$n]["username"]) . "</a></b> (" . $active[$n]["t"] . ")";}
?>
