...
if ($job_user == $userref) {
    ...
    $breadcrumbs = [
        ...
        ['title' => $lang["job_configure"] . ": " . $lang["job_list_checksums"]]
    ];
} else {
...
// Process form input
$save       = getval('save', '');
$recreate   = (bool) getval('recreate', false);
...
if ($save != '' && enforcePostRequest(false)) {
    ...
    if ($valid_data) {
        ...
        // Create the job
        $job_added = job_queue_add("update_checksums", $job_data);
        
        if (!is_int_loose($job_added)) {
            $job_add_error = true;
        } else {
            // Log that job has been added to user activity log, and then redirect
            log_activity("Added update_checksums job $job_added", 
                            LOG_CODE_JOB_ADDED,
...
<div class='BasicsBox'>
    <h1>
        <?php
        echo escape($lang["job_configure"] . ": " . $lang["job_list_checksums"]);
    ...
    <form method="post" id="update_checksums_form" action="<?php echo generateURL("{$baseurl_short}pages/offline_jobs/update_checksums.php", ["job_user" => $job_user]); ?>">
        <?php generateFormToken("update_checksums_form"); ?>
        <div class="Question">
            <label for="recreate"><?php echo escape($lang["oj_checksums_recreate"]); ?></label>
            <input name="recreate" id="recreate" type="checkbox" value="1"<?php echo ($recreate ? 'checked="checked"' : ''); ?>>
            <div class="FormHelp JobFormHelp">
                <div class="FormHelpInner"><?php echo escape($lang["oj_checksums_recreate_help"]); ?></div>
            </div> 
